...
2026-09-04 终裁——自动 refine 默认关闭（保留在 `RunOptions.autoRefine` 后），
依据 uplift-v2：analyze 尺子看穿表达（04-midmatch 矩阵零翻转、两轮复现），自动
腿改善盲评质量（70.8%）但不动分数；提分叙事落在问题卡片（新事实，+1~+4 已验
证）；两次用户免费 refine 恢复，B3 计费毛边与 per-IP 翻倍随之消失。
